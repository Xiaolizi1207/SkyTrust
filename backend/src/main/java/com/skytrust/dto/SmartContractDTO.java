 ...
 */
@Data
@Schema(description = "智能合约数据传输对象")
public class SmartContractDTO {

    @Schema(description = "合约名称", example = "租赁合约V1", required = true)
    @NotBlank(message = "合约名称不能为空")
    @Size(max = 100, message = "合约名称长度不能超过100个字符")
    private String contractName;

    @Schema(description = "合约地址（唯一）", example = "0x742d35Cc6634C0532925a3b844Bc9e", required = true)
    @NotBlank(message = "合约地址不能为空")
    @Size(max = 100, message = "合约地址长度不能超过100个字符")
    private String contractAddress;

    @Schema(description = "合约ABI（JSON格式）", example = "[{\"inputs\":[],\"name\":\"getBalance\",\"outputs\":[{\"name\":\"\",\"type\":\"uint256\"}]}]", required = true)
    @NotBlank(message = "合约ABI不能为空")
    @Size(max = 10000, message = "合约ABI长度不能超过10000个字符")
    private String contractAbi;

    @Schema(description = "合约字节码", example = "0x608060405234801561001057600080fd5b5060", required = true)
    @NotBlank(message = "合约字节码不能为空")
    @Size(max = 10000, message = "合约字节码长度不能超过10000个字符")
    private String contractBytecode;

    @Schema(description = "合约类型（0-租赁合约，1-支付合约，2-保险合约，3-设备合约）", example = "0", required = true)
    @NotNull(message = "合约类型不能为空")
    private Integer contractType;

    @Schema(description = "合约版本", example = "1.0.0", required = true)
    @NotBlank(message = "合约版本不能为空")
    @Size(max = 20, message = "合约版本长度不能超过20个字符")
    private String version;

    @Schema(description = "部署交易哈希", example = "0x1234567890abcdef1234567890abcdef1234567890abcdef1234567890abcdef", required = true)
    @NotBlank(message = "部署交易哈希不能为空")
    @Size(max = 100, message = "部署交易哈希长度不能超过100个字符")
    private String deployTxHash;

    @Schema(description = "部署者地址", example = "0x742d35Cc6634C0532925a3b844Bc9e", required = true)
    @NotBlank(message = "部署者地址不能为空")
    @Size(max = 100, message = "部署者地址长度不能超过100个字符")
    private String deployerAddress;

    @Schema(description = "部署网络ID（链ID）", example = "1", required = true)
    @NotNull(message = "部署网络ID不能为空")
    private Integer chainId;

    @Schema(description = "部署时间", example = "2023-10-01T10:00:00", required = true)
    @NotNull(message = "部署时间不能为空")
    private LocalDateTime deployTime;

    @Schema(description = "合约状态（0-未激活，1-已激活，2-已暂停，3-已终止）", example = "1", required = true)
    @NotNull(message = "合约状态不能为空")
    private Integer contractStatus;

    @Schema(description = "是否可升级（0-否，1-是）", example = "true", required = true)
    ...
    @Schema(description = "合约拥有者地址", example = "0x742d35Cc6634C0532925a3b844Bc9e", required = true)
