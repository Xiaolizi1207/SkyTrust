...
public class SystemConfigDTO {
    ...
    @ApiModelProperty(value = "配置键（唯一）", example = "app.name", required = true)
    @NotBlank(message = "配置键不能为空")
    @Size(max = 100, message = "配置键长度不能超过100个字符")
    private String configKey;

    @ApiModelProperty(value = "配置值", example = "SkyTrust无人机租赁平台", required = true)
    @NotBlank(message = "配置值不能为空")
    @Size(max = 2000, message = "配置值长度不能超过2000个字符")
    private String configValue;

    @ApiModelProperty(value = "配置类型（0-系统配置，1-业务配置，2-区块链配置，3-AI配置，4-物联网配置）", example = "0", required = true)
    ...
    @ApiModelProperty(value = "是否可修改（0-否，1-是）", example = "true", required = true)
