 ...
 */
@Data
@ApiModel(description = "租赁订单数据传输对象")
public class RentalOrderDTO {
    ...
    @ApiModelProperty(value = "租赁开始时间", example = "2023-10-01T10:00:00", required = true)
    @NotNull(message = "租赁开始时间不能为空")
    @Future(message = "租赁开始时间必须是将来的时间")
    private LocalDateTime startTime;

    @ApiModelProperty(value = "租赁结束时间", example = "2023-10-01T12:00:00", required = true)
    @NotNull(message = "租赁结束时间不能为空")
    @Future(message = "租赁结束时间必须是将来的时间")
    private LocalDateTime endTime;

    @ApiModelProperty(value = "保险费用（元）", example = "50.00", required = true)
