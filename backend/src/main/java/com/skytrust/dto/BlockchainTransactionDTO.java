 ...
 */
@Data
@ApiModel(description = "区块链交易记录数据传输对象")
public class BlockchainTransactionDTO {

    @ApiModelProperty(value = "智能合约地址", example = "0x742d35Cc6634C0532925a3b844Bc9e", required = true)
    @NotBlank(message = "智能合约地址不能为空")
    @Size(max = 100, message = "智能合约地址长度不能超过100个字符")
    private String contractAddress;

    @ApiModelProperty(value = "交易发起方地址", example = "0x742d35Cc6634C0532925a3b844Bc9e", required = true)
    @NotBlank(message = "发起方地址不能为空")
    @Size(max = 100, message = "发起方地址长度不能超过100个字符")
    private String fromAddress;

    @ApiModelProperty(value = "交易接收方地址", example = "0x742d35Cc6634C0532925a3b844Bc9e", required = true)
    @NotBlank(message = "接收方地址不能为空")
    @Size(max = 100, message = "接收方地址长度不能超过100个字符")
    private String toAddress;

    @ApiModelProperty(value = "交易金额（ETH/代币数量）", example = "1.5", required = true)
    @NotNull(message = "交易金额不能为空")
    @DecimalMin(value = "0.00000001", message = "交易金额必须大于0")
    @Digits(integer = 10, fraction = 8, message = "交易金额格式不正确")
    private BigDecimal amount;

    @ApiModelProperty(value = "交易gas费用", example = "0.001", required = true)
    ...
    @ApiModelProperty(value = "交易类型（0-租赁合约创建，1-支付，2-保险，3-设备状态更新）", example = "1", required = true)
    @NotNull(message = "交易类型不能为空")
    private Integer txType;

    @ApiModelProperty(value = "关联业务ID（如订单ID、支付ID等）", example = "1", required = true)
    @NotNull(message = "关联业务ID不能为空")
    private Long businessId;

    @ApiModelProperty(value = "关联业务类型（order-订单，payment-支付，insurance-保险）", example = "payment", required = true)
    ...
    @ApiModelProperty(value = "网络ID（链ID）", example = "1", required = true)
