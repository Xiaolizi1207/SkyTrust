...
public class DeviceDTO {
    ...
    @ApiModelProperty(value = "设备名称", example = "大疆无人机 Mavic 3", required = true)
    @NotBlank(message = "设备名称不能为空")
    @Size(max = 100, message = "设备名称长度不能超过100个字符")
    private String deviceName;

    @ApiModelProperty(value = "设备型号", example = "Mavic 3 Pro", required = true)
    @NotBlank(message = "设备型号不能为空")
    @Size(max = 50, message = "设备型号长度不能超过50个字符")
    private String model;

    @ApiModelProperty(value = "设备序列号（唯一）", example = "DJI20231001001", required = true)
    @NotBlank(message = "设备序列号不能为空")
    @Size(max = 100, message = "设备序列号长度不能超过100个字符")
    private String serialNumber;

    @ApiModelProperty(value = "设备状态（0-离线，1-在线，2-飞行中，3-维修中，4-已报废）", example = "1", required = true)
    ...
    @ApiModelProperty(value = "设备所有者ID", example = "1", required = true)
    @NotNull(message = "设备所有者ID不能为空")
    private Long ownerId;

    @ApiModelProperty(value = "租赁价格（元/小时）", example = "200.00", required = true)
    @NotNull(message = "租赁价格不能为空")
    @DecimalMin(value = "0.0", message = "租赁价格不能小于0")
    @Digits(integer = 10, fraction = 2, message = "租赁价格格式不正确")
    private BigDecimal rentalPrice;

    @ApiModelProperty(value = "保险费用（元/次）", example = "50.00", required = true)
